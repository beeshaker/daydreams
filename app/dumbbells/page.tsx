...
const PAGE_STYLE = `
  ...
  /* ---------------------------------------------------------------- nav */
  /* Matches Daydreams' SiteHeader structurally (logo + wordmark, a row of
     in-page section links, a cross-brand link, sticky + opaque + blurred)
     rebuilt in this page's own dark/crimson tokens rather than importing
     that component, since this page is raw HTML (see file-level comment
     above) to avoid drifting from the verified build. */
  .nav {
  ...
  .nav__row {
    ...
    padding: clamp(.8rem, 2vw, 1.1rem) var(--sc-gutter);
  }
  .nav__mark {
  ...
  .nav__logo { width: 2rem; height: 2rem; border-radius: 50%; object-fit: cover; }
  .nav__actions { display: flex; align-items: center; gap: var(--sc-5); }
  .nav__other {
    display: none; font-family: var(--sc-font-text); font-size: var(--sc-t-sm);
    color: var(--sc-ink-soft); text-decoration: none;
  }
  .nav__other:hover { color: var(--sc-ink); }
  @media (min-width: 640px) { .nav__other { display: inline-flex; align-items: center; gap: .3rem; } }
  .nav__cta {
    flex-shrink: 0;
  ...
  .nav__cta:focus-visible { outline: 2px solid var(--sc-ink); outline-offset: 3px; }
  .nav__links {
    display: flex; gap: var(--sc-5); overflow-x: auto;
    padding: 0 var(--sc-gutter) .85rem;
    font-family: var(--sc-font-text); font-size: var(--sc-t-sm);
  }
  .nav__links a { flex-shrink: 0; padding: .2rem 0; color: var(--sc-ink-soft); text-decoration: none; }
  ...
  /* No upper bound: on wide/ultrawide screens the fixed-width cards below
     would otherwise stop growing while the viewport keeps growing, so past
     ~1920px there'd be nothing left to pan through (the rail would fit
     entirely on screen while the section is still pinned for its full
     scroll span). Scaling with vw indefinitely keeps the rail wider than
     the viewport at any screen size. */
  .rail__lead { flex: 0 0 max(16rem, 30vw); display: flex; flex-direction: column; justify-content: center; }
  .rail__lead h2 { margin: 0 0 var(--sc-4); }
  .item {
    flex: 0 0 max(15rem, 24vw); display: flex; flex-direction: column; gap: var(--sc-4);
  }
  .item__frame {
  ...
  .rail__note { flex: 0 0 max(14rem, 22vw); display: flex; flex-direction: column; justify-content: center; }
  ...
  /* ------------------------------------------------- schedule / warmth glow */
  .schedule, .warmth { position: relative; overflow: clip; }
  .schedule-glow {
    position: absolute; inset: -30% -10%; z-index: -1; pointer-events: none;
    background: radial-gradient(42% 55% at 82% 30%,
      color-mix(in oklab, var(--sc-accent) 20%, transparent), transparent 70%);
    will-change: transform;
  }
  .warmth-glow {
    position: absolute; inset: -30% -10%; z-index: -1; pointer-events: none;
    background: radial-gradient(46% 60% at 18% 65%,
  ...
  .site-foot {
    border-top: 1px solid var(--sc-hairline);
    background: color-mix(in oklab, var(--sc-canvas) 96%, black 4%);
  }
  .site-foot__row {
  ...
  .site-foot__mark {
    font-family: var(--sc-font-display); font-weight: 800; font-size: var(--sc-t-lg);
    letter-spacing: var(--sc-track-wide); text-transform: uppercase; color: var(--sc-ink);
  }
  .site-foot__brand .sc-body { margin-top: var(--sc-3); max-width: 26rem; }
  .site-foot h3 {
    margin: 0 0 var(--sc-4); font-family: var(--sc-font-text); font-weight: 600; font-size: var(--sc-t-sm);
    text-transform: uppercase; letter-spacing: var(--sc-track-wide); color: var(--sc-ink-soft);
  }
  .site-foot p { margin: 0 0 var(--sc-3); font-size: var(--sc-t-sm); color: var(--sc-ink-soft); line-height: var(--sc-leading-body); }
  .site-foot p strong { color: var(--sc-ink); font-weight: 600; }
  .site-foot a { color: var(--sc-ink-soft); text-decoration: none; }
  .site-foot a:hover { color: var(--sc-accent); }
  .site-foot__bottom {
    display: flex; flex-wrap: wrap; justify-content: space-between; gap: var(--sc-4);
    max-width: var(--sc-maxw); margin-inline: auto;
    padding: var(--sc-5) var(--sc-gutter) var(--sc-7);
    border-top: 1px solid var(--sc-hairline);
    font-size: var(--sc-t-xs); color: var(--sc-ink-soft);
  }
  .site-foot__bottom nav { display: flex; gap: var(--sc-5); }
  @media (max-width: 640px) {
    .site-foot__row { grid-template-columns: 1fr; }
  }
...
<header class="nav">
  <div class="nav__row">
    <a class="nav__mark" href="#top" aria-label="Dumbbells home">
      <img class="nav__logo" src="/branding/logo.png" alt="">
      <span>Dumbbells</span>
    </a>
    <div class="nav__actions">
      <a class="nav__other" href="/daydreams/site">Visit Daydreams <span aria-hidden="true">&rarr;</span></a>
      <a class="nav__cta" href="#join">Book a Session</a>
    </div>
  </div>
  <nav class="nav__links" aria-label="Dumbbells sections">
    <a href="#range">Classes</a>
...
<main id="top">
  ...
  <!-- 4 · CURIOSITY : pan -->
  <section id="range" data-sc-act="pan" data-sc-span="3.2" data-sc-drift="#120e0c">
    <div data-sc-stage>
      <div class="rail-progress" aria-hidden="true"><span class="rail-progress__fill"></span></div>
  ...
  <section id="schedule" class="sc-section schedule" data-sc-act="flow" data-sc-drift="#100d0c">
    <div class="schedule-glow" data-sc-parallax="-0.25" aria-hidden="true"></div>
    <div class="sc-wrap">
  ...
  <section id="members" class="sc-section warmth" data-sc-act="flow" data-sc-drift="#0e0b0a">
    <div class="warmth-glow" data-sc-parallax="-0.2" aria-hidden="true"></div>
    <div class="sc-wrap">
      <div class="sc-stack" data-sc-in data-sc-stagger="60">
  ...
  <!-- BOOK : flow. The old pinned closing photo now leads straight into the
...
<footer class="site-foot">
  <div class="site-foot__row">
    <div class="site-foot__brand">
      <span class="site-foot__mark">Dumbbells</span>
      <p class="sc-body">Strength training for people with a life outside the gym.</p>
    </div>
    <div class="site-foot__hours">
      <h3>Hours</h3>
      <p>Mon &ndash; Fri<br><strong>8:30 AM &ndash; 6:00 PM</strong></p>
      <p>Saturday<br><strong>9:00 AM &ndash; 1:00 PM</strong></p>
    </div>
    <div class="site-foot__contact">
      <h3>Visit</h3>
      <p>41 Gurviellia Grove, Nairobi</p>
      <p><a href="tel:+254180117040">0180 117040</a></p>
      <p><a href="mailto:daydreamsanddumbbells@gmail.com">daydreamsanddumbbells@gmail.com</a></p>
    </div>
  </div>
  <div class="site-foot__bottom">
    <p>&copy; Dumbbells. All rights reserved.</p>
    <nav aria-label="Legal">
      <a href="/dumbbells/privacy">Privacy Policy</a>
      <a href="/dumbbells/terms">Terms of Service</a>
    </nav>
  </div>
</footer>
`;
